/**
 * 民泊清掃予定 自動割り当てシステム v8
 *
 * v7からの変更点:
 *   - 異常時のメール通知（トークン失効・実行エラー・未割当）
 *   - キープアライブトリガー（Beds24トークンの失効防止と稼働監視）
 *   - 稼働状態の確認メニューと、設定シートへの実行結果記録
 */
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem('🩺 稼働状態を確認', 'checkSystemStatus')
...
function setupSpreadsheet() {
  ...
  stg.getRange('A18').setValue('最終自動実行').setFontWeight('bold');
  stg.getRange('B18').setValue('（未実行）').setFontColor('#999999');
  stg.getRange('B18').setNote('日次トリガーが最後に動いた日時と結果。\n異常時はメールでも通知されます。');
...
function setupBeds24Auth() {
  ...
  try {
    ...
    props.setProperty('BEDS24_TOKEN_LAST_OK', String(Date.now()));
...
// --- アクセストークン取得（自動更新） ---
//
// forceRefresh=true の場合、キャッシュが有効でも必ず
// /authentication/token を呼ぶ。リフレッシュトークンは
// 30日間使われないと失効するため、キープアライブで使用する。
function getBeds24AccessToken_(forceRefresh) {
  var props = PropertiesService.getScriptProperties();
  var refreshToken = props.getProperty('BEDS24_REFRESH_TOKEN');
  if (!refreshToken) {
    setConnectionState_('未接続', '#CC0000', 'リフレッシュトークンが保存されていません。');
    return null;
  }
  ...
  if (code !== 200) {
    var detail = 'HTTP ' + code + ' ' + res.getContentText();
    Logger.log('Beds24 token error: ' + detail);

    // 401/403 = リフレッシュトークンが失効・取り消された（再接続が必要）
    // それ以外 = 一時的な障害の可能性が高いのでトークンは保持する
    if (code === 401 || code === 403) {
      setConnectionState_('要再接続 ✗', '#CC0000', detail);
      notifyOwner_(
        '【清掃予定管理】Beds24の再接続が必要です',
        'Beds24のリフレッシュトークンが失効しました。予約データの自動取得が停止しています。\n\n' +
        '■ 復旧手順\n' +
        '1. Beds24管理画面 → (SETTINGS) MARKETPLACE → API で招待コードを発行\n' +
        '   （スコープに bookings と properties を含める / 有効期限は24時間）\n' +
        '2. スプレッドシート → 🧹 清掃管理 → 🔑 Beds24 API接続設定 → 招待コードを入力\n\n' +
        '■ 詳細\n' + detail + '\n\n' +
        '※ 復旧するまで、清掃予定はカレンダーに反映されません。',
        'auth_expired'
      );
    } else {
      setConnectionState_('接続エラー（一時的）', '#CC6600', detail);
      notifyOwner_(
        '【清掃予定管理】Beds24への接続に失敗しています',
        'Beds24 APIへの接続に失敗しました。一時的な障害の可能性があります。\n\n' +
        '翌日の自動実行で復旧しない場合は、スプレッドシートの\n' +
        '「🧹 清掃管理 → 🩺 稼働状態を確認」から状態を確認してください。\n\n' +
        '■ 詳細\n' + detail,
        'auth_error'
      );
    }
  ...
  if (!body.token) {
    ...
    setConnectionState_('接続エラー', '#CC6600', 'トークンが返却されませんでした。');
    return null;
  }
  ...
  // リフレッシュトークンを最後に使えた日時（30日で失効するため記録）
  props.setProperty('BEDS24_TOKEN_LAST_OK', String(Date.now()));
  setConnectionState_('接続済み ✓', '#006600', '');
  return body.token;
}
...
function fetchBeds24Bookings_() {
  ...
  for (var i = 0; i < allBookings.length; i++) {
    ...
    if (i === 0) Logger.log('Beds24 booking keys: ' + Object.keys(b).join(', '));
    var title = String(b.guestTitle || b.title || '').trim();
...
function runAllAuto() {
  try {
    ...
    if (fetchCount === 0) {
      ...
      recordRunResult_(false, 'Beds24から予約データが0件でした。');
      notifyOwner_(
        '【清掃予定管理】予約データが0件でした',
        'Beds24から取得した予約データが0件だったため、割り当てを行いませんでした。\n\n' +
        '本当に予約が0件であれば問題ありませんが、通常はユニットマッピングの\n' +
        '設定漏れや取得条件の誤りが原因です。\n' +
        '「🧹 清掃管理 → 🔍 Beds24 roomId確認」で取得状況を確認してください。',
        'zero_bookings'
      );
      return;
    }
    ...
    if (assignments.length === 0) {
      ...
      recordRunResult_(true, '取得 ' + fetchCount + '件 / 割り当て対象なし');
      return;
    }
    writeResults_(assignments);
    writeGanttChart_(assignments);
    ...
    Logger.log(msg);
    recordRunResult_(true, msg.replace(SYSTEM_TAG + ' ', ''));

    // 未割当が残っている場合は管理者に知らせる（外注手配が必要なため）
    if (una > 0) {
      notifyOwner_(
        '【清掃予定管理】未割当が ' + una + '件あります',
        '割り当てが完了しましたが、担当が決まっていない清掃が ' + una + '件あります。\n' +
        '「割り当て結果」シートでステータスが「要確認」の行をご確認ください。\n\n' +
        msg.replace(SYSTEM_TAG + ' ', ''),
        'unassigned'
      );
    }
    ...
    recordRunResult_(false, e.message);
    notifyOwner_(
      '【清掃予定管理】自動実行がエラーで停止しました',
      '日次の自動実行がエラーで停止しました。清掃予定はカレンダーに反映されていません。\n\n' +
      '■ エラー内容\n' + e.message + '\n\n' +
      '■ 確認手順\n' +
      '1. スプレッドシート → 🧹 清掃管理 → 🩺 稼働状態を確認\n' +
      '2. Apps Scriptエディタ → 左メニュー「実行数」で詳細を確認\n\n' +
      '※ 復旧するまで毎日この状態が続きます（通知は12時間に1回までです）。',
      'run_error'
    );
    try {
      showAlert_('エラー', e.message);
...
function setupDailyTrigger() {
  ...
  // キープアライブは全自動実行と別時刻に置く。
  // 全自動実行が壊れても、トークンの失効防止と異常通知が生き残る。
  var keepHour = (hour + 12) % 24;
  ScriptApp.newTrigger('beds24KeepAlive')
    .timeBased()
    .everyDays(1)
    .atHour(keepHour)
    .create();
  ...
  showAlert_('トリガー設定完了',
    ...
    '2. 割り当てアルゴリズム実行\n' +
    '3. Googleカレンダーに自動反映\n\n' +
    'あわせて毎日 ' + keepHour + '時に見張り役（キープアライブ）が動きます。\n' +
    'Beds24トークンの失効防止と、異常時のメール通知を行います。\n' +
    '通知先: ' + (Session.getEffectiveUser().getEmail() || '（取得できません）'));
}
...
function removeDailyTrigger_(silent) {
  ...
  for (var i = 0; i < triggers.length; i++) {
    ...
    if (fn === 'runAllAuto' || fn === 'beds24KeepAlive') {
...
// 通知・稼働監視
//
// トリガー実行はUIがないため、異常があっても管理者が
// 気づけない。メール通知と稼働状態の記録で可視化する。
// ============================================================

// 同じ種類の通知を送る最小間隔（時間）
var NOTIFY_THROTTLE_HOURS = 12;
// 最後に正常終了してから、この日数を超えたら警告する
var STALE_RUN_ALERT_DAYS = 3;
// リフレッシュトークンの失効までの日数（Beds24仕様）
var REFRESH_TOKEN_EXPIRE_DAYS = 30;
// 失効の何日前から警告するか
var TOKEN_WARN_DAYS = 20;

function notifyOwner_(subject, body, throttleKey) {
  try {
    var props = PropertiesService.getScriptProperties();

    if (throttleKey) {
      var key  = 'NOTIFY_LAST_' + throttleKey;
      var last = Number(props.getProperty(key) || 0);
      if (Date.now() - last < NOTIFY_THROTTLE_HOURS * 3600 * 1000) return;
      props.setProperty(key, String(Date.now()));
    }

    var to = Session.getEffectiveUser().getEmail();
    if (!to) { Logger.log('notifyOwner_: 宛先メールアドレスを取得できません'); return; }

    var ssUrl = '';
    try { ssUrl = SpreadsheetApp.getActiveSpreadsheet().getUrl(); } catch (e) {}
    if (ssUrl) body += '\n\n■ スプレッドシート\n' + ssUrl;

    MailApp.sendEmail(to, subject, body);
    Logger.log('notifyOwner_: 通知を送信しました → ' + to + ' / ' + subject);
  } catch (e) {
    Logger.log('notifyOwner_ エラー: ' + e.message);
  }
}

// 設定シートの接続状態セル（B15）を更新する
function setConnectionState_(text, color, note) {
  try {
    var stg = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_SETTINGS);
    if (!stg) return;
    var cell = stg.getRange('B15');
    cell.setValue(text).setFontColor(color || '#000000');
    cell.setNote(note ? (formatDateTime_(new Date()) + '\n' + note) : '');
  } catch (e) {
    Logger.log('setConnectionState_ エラー: ' + e.message);
  }
}

// 全自動実行の結果を設定シートに記録する
function recordRunResult_(ok, message) {
  var props = PropertiesService.getScriptProperties();
  if (ok) props.setProperty('LAST_SUCCESS_RUN', String(Date.now()));

  try {
    var stg = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_SETTINGS);
    if (!stg) return;
    // 既存シートにも自己修復的にラベルを書き込む
    stg.getRange('A18').setValue('最終自動実行').setFontWeight('bold');
    stg.getRange('B18')
      .setValue(formatDateTime_(new Date()) + (ok ? ' 正常終了' : ' エラー'))
      .setFontColor(ok ? '#006600' : '#CC0000')
      .setNote(message || '');
  } catch (e) {
    Logger.log('recordRunResult_ エラー: ' + e.message);
  }
}

// --- キープアライブ（日次トリガー） ---
//
// 全自動実行とは別のトリガーで動く見張り役。
//   1. リフレッシュトークンを能動的に使って失効を防ぐ
//   2. 全自動実行が何日も成功していない場合に管理者へ通知する
// 全自動実行が壊れても、こちらが生きていれば異常に気づける。
function beds24KeepAlive() {
  var props = PropertiesService.getScriptProperties();

  // 1. トークンを強制リフレッシュ（失効の起点となる「未使用期間」をリセット）
  var token = getBeds24AccessToken_(true);
  if (token) {
    Logger.log(SYSTEM_TAG + ' キープアライブ: トークン更新OK');
  } else {
    Logger.log(SYSTEM_TAG + ' キープアライブ: トークン更新に失敗');
    // 通知は getBeds24AccessToken_ 内で送信済み
  }

  // 2. 全自動実行が滞っていないか確認
  var lastSuccess = Number(props.getProperty('LAST_SUCCESS_RUN') || 0);
  if (lastSuccess > 0) {
    var days = Math.floor((Date.now() - lastSuccess) / (24 * 3600 * 1000));
    if (days >= STALE_RUN_ALERT_DAYS) {
      notifyOwner_(
        '【清掃予定管理】' + days + '日間、自動実行が成功していません',
        '最後に全自動実行が正常終了したのは ' + formatDateTime_(new Date(lastSuccess)) + ' です。\n' +
        'それ以降、' + days + '日間 正常終了していません。\n\n' +
        '■ 確認手順\n' +
        '1. スプレッドシート → 🧹 清掃管理 → 🩺 稼働状態を確認\n' +
        '2. Apps Scriptエディタ → 左メニュー「実行数」でエラー内容を確認\n' +
        '3. トリガーが消えている場合は「⏰ 日次自動実行を設定」を実行し直す\n\n' +
        '※ この間、清掃予定はカレンダーに反映されていません。',
        'stale_run'
      );
    }
  }

  // 3. リフレッシュトークンの失効が近づいていないか確認
  var lastTokenOk = Number(props.getProperty('BEDS24_TOKEN_LAST_OK') || 0);
  if (lastTokenOk > 0) {
    var tokenDays = Math.floor((Date.now() - lastTokenOk) / (24 * 3600 * 1000));
    if (tokenDays >= TOKEN_WARN_DAYS) {
      notifyOwner_(
        '【清掃予定管理】Beds24トークンの失効が近づいています',
        'Beds24のリフレッシュトークンが ' + tokenDays + '日間 使用されていません。\n' +
        REFRESH_TOKEN_EXPIRE_DAYS + '日間 使用されないと失効し、招待コードからの再接続が必要になります。\n\n' +
        '残り約 ' + (REFRESH_TOKEN_EXPIRE_DAYS - tokenDays) + '日です。\n' +
        'スプレッドシート → 🧹 清掃管理 → 🩺 稼働状態を確認 から状態を確認してください。',
        'token_stale'
      );
    }
  }
...
function checkSystemStatus() {
  ...
  // トリガー
  var triggers = ScriptApp.getProjectTriggers();
  var mainCount = 0, keepCount = 0;
  for (var i = 0; i < triggers.length; i++) {
    var fn = triggers[i].getHandlerFunction();
    if (fn === 'runAllAuto')      mainCount++;
    if (fn === 'beds24KeepAlive') keepCount++;
  }
  lines.push('■ 自動実行トリガー');
  lines.push('  全自動実行: ' + (mainCount > 0 ? '設定済み (' + mainCount + ')' : '⚠ 未設定'));
  lines.push('  キープアライブ: ' + (keepCount > 0 ? '設定済み (' + keepCount + ')' : '⚠ 未設定'));
  if (mainCount === 0 || keepCount === 0) {
    lines.push('  → 「⏰ 日次自動実行を設定」を実行してください');
  }
  ...
  if (lastSuccess > 0) {
    var days = Math.floor((Date.now() - lastSuccess) / (24 * 3600 * 1000));
    lines.push('  ' + formatDateTime_(new Date(lastSuccess)) + '（' + days + '日前）');
    if (days >= STALE_RUN_ALERT_DAYS) lines.push('  ⚠ 自動実行が滞っています');
  } else {
    lines.push('  記録なし（まだ一度も自動実行が完了していません）');
  }

  // Beds24トークン
  lines.push('');
  lines.push('■ Beds24 API');
  var hasRefresh = !!props.getProperty('BEDS24_REFRESH_TOKEN');
  lines.push('  リフレッシュトークン: ' + (hasRefresh ? 'あり' : '⚠ なし（再接続が必要）'));
  var lastTokenOk = Number(props.getProperty('BEDS24_TOKEN_LAST_OK') || 0);
  if (lastTokenOk > 0) {
    var tokenDays = Math.floor((Date.now() - lastTokenOk) / (24 * 3600 * 1000));
    lines.push('  最後に更新できた日: ' + formatDateTime_(new Date(lastTokenOk)) +
               '（' + tokenDays + '日前）');
    lines.push('  失効まで残り: 約 ' + Math.max(0, REFRESH_TOKEN_EXPIRE_DAYS - tokenDays) + '日');
  } else {
    lines.push('  最後に更新できた日: 記録なし');
  }

  lines.push('');
  lines.push('■ 通知先');
  lines.push('  ' + (Session.getEffectiveUser().getEmail() || '（取得できません）'));

  showAlert_('稼働状態', lines.join('\n'));
}
