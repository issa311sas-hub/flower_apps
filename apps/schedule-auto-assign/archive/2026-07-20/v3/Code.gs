/**
 * 民泊清掃予定 自動割り当てシステム v3
 *
 * v2からの変更点:
 *   - 入力データ形式変更: 予約ID, タイトル, チェックアウト日, ユニット
 *   - 同期データベースシート追加（差分検出用）
 *   - カレンダー書き込みを差分のみに最適化（API呼び出し大幅削減）
 *   - カレンダー読み込みを新規予約がある日のみに最適化
 *   - 手動カレンダー編集を保持（既存イベントは上書きしない）
 *   - カレンダー出力にタイトル（特殊要望）を含む
 */
...
function setupSpreadsheet() {
  ...
  db.clear();
  db.getRange('A1:G1')
    .setValues([['予約ID', 'チェックアウト日', 'ユニット', 'タイトル', '担当', 'イベントID', '最終同期']])
  ...
  db.setColumnWidth(4, 150);
  db.setColumnWidth(5, 100);
  db.setColumnWidth(6, 280);
  db.setColumnWidth(7, 150);
...
// ============================================================
// 予約データ読み込み（v3: 予約ID, タイトル, チェックアウト日, ユニット）
// ============================================================
...
function writeDatabase_(records) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    rows.push([r.bookingId, r.dateStr, r.unit, r.title, r.staff, r.eventId || '', r.lastSync || now]);
  }
...
function computeDiff_(reservations, db) {
  ...
  for (var i = 0; i < reservations.length; i++) {
    ...
    if (!prev) {
      added.push(r);
    } else if (prev.dateStr !== r.dateStr || prev.unit !== r.unit || prev.title !== r.title) {
...
// ============================================================
// マッチングアルゴリズム（v3: 差分対応・既存割り当て保持）
//
// 既存の割り当て（DB内の未変更予約）はそのまま保持し、
// 新規・変更分のみ割り当てアルゴリズムを実行する。
// これにより手動でのカレンダー編集が上書きされない。
// ============================================================
function doMatching_() {
  ...
  for (var u = 0; u < diff.unchanged.length; u++) {
    ...
    var a = {
      ...
      date:      uc.newData.date,
      dateStr:   uc.newData.dateStr,
      dayName:   DAY_NAMES[uc.newData.dow],
      unit:      uc.newData.unit,
      title:     uc.newData.title,
      staff:     uc.oldData.staff,
      status:    statusFor_(uc.oldData.staff)
    };
    allAssignments.push(a);

    if (!existingByDate[a.dateStr]) existingByDate[a.dateStr] = {};
    existingByDate[a.dateStr][a.staff] = (existingByDate[a.dateStr][a.staff] || 0) + 1;
  }
  ...
  if (toAssign.length > 0) {
    var datesNeeded = {};
    for (var t = 0; t < toAssign.length; t++) datesNeeded[toAssign[t].dateStr] = true;
    ...
    for (var di = 0; di < dateKeys.length; di++) {
      ...
      for (var a3 = 0; a3 < rclAlloc; a3++, unitIdx++) {
        allAssignments.push(makeAssignFromBooking_(newItems[unitIdx], rclInfo ? rclInfo.name : 'Rクリーン'));
      }
...
// ============================================================
// カレンダーに差分反映（v3: 差分のみ書き込み）
//
// DB（前回同期状態）と割り当て結果シートを比較し、
// 新規・変更分のみイベント作成、削除分のみイベント削除する。
// 既存の（変更のない）イベントには一切触れないため、
// 手動でカレンダーを編集した内容も保持される。
// ============================================================
function doSyncToCalendar_() {
  ...
  for (var i = 0; i < resultData.length; i++) {
    ...
    current[bid] = {
      bookingId: bid,
      dateStr:   String(resultData[i][1]).trim(),
  ...
  for (var d = 0; d < dbKeys.length; d++) {
    ...
    } else if (dbRec.dateStr !== curRec.dateStr || dbRec.unit !== curRec.unit ||
               dbRec.title !== curRec.title || dbRec.staff !== curRec.staff) {
      if (dbRec.eventId) toDelete.push(dbRec);
      toCreate.push(curRec);
      newDb[dbBid] = {
        bookingId: dbBid, dateStr: curRec.dateStr, unit: curRec.unit,
  ...
  for (var c = 0; c < curKeys.length; c++) {
    ...
    if (!db[curBid]) {
      ...
      newDb[curBid] = {
        bookingId: curBid, dateStr: current[curBid].dateStr, unit: current[curBid].unit,
  ...
  for (var cr = 0; cr < toCreate.length; cr++) {
    ...
    var evTitle = rec.title
      ...
      : rec.staff + '⇒' + rec.unit;
    var evDate = parseDateStr_(rec.dateStr);
